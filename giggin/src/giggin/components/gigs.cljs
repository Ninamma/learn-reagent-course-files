...
(defn gigs
  []
  (let [modal (r/atom {:active false :gig nil})
       ...
       [:div.gigs
        [:button.add-gig
         ...
         [:div.add__title
          [:i.icon.icon--plus]
          [:p "Add gig"]]]
        [gig-editor modal values toggle-modal upsert-gig]
        (for [{:keys [id img title price desc] :as gig} (vals @state/gigs)]
          [:div.gig {:key id}
           [:img.gig__artwork.gig__edit
            {:src img
             :alt title
             :on-click #(toggle-modal {:active true :gig gig})}]
           [:div.gig__body
            [:div.gig__title
